...
const ProfileDropdown = () => {
  ...
  const displayName = user?.full_name || user?.fullName || (isAdmin ? 'Admin' : 'Technician');
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      >
        <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-blue-700 to-indigo-600 flex items-center justify-center text-white font-bold text-xs uppercase shadow-md border border-blue-500/30">
          {initial}
        </div>
        <div className="hidden lg:block text-left pr-1 leading-tight">
          <p className="text-xs font-semibold text-slate-200">{displayName}</p>
          <span className="text-[10px] text-blue-400 font-medium capitalize">{user?.role || (isAdmin ? 'admin' : 'technician')}</span>
        </div>
      </button>
      ...
      {isOpen && (
        <div className="absolute right-0 mt-2 w-56 glass-panel bg-slate-900/95 rounded-2xl shadow-2xl border border-slate-800 p-2 z-50 animate-enter">
          <div className="px-3 py-2 border-b border-slate-800 mb-1">
            ...
            <p className="text-[11px] text-slate-400 truncate">{user?.email || 'admin@converge.it'}</p>
            <div className="mt-1 inline-flex items-center space-x-1 px-2 py-0.5 rounded-md bg-blue-500/20 text-blue-300 text-[10px] font-semibold">
              <Shield className="w-3 h-3" />
              <span className="capitalize">{user?.role || (isAdmin ? 'admin' : 'technician')}</span>
            </div>
          </div>
