    ...
    const [
      ...
      query(`
        SELECT 
          ...
          COUNT(*) FILTER (WHERE sla_deadline >= NOW() AND sla_deadline <= NOW() + INTERVAL '3 hours' AND status NOT IN ('resolved','closed')) AS sla_at_risk,
      ...
      query(`
        SELECT
          COUNT(*) FILTER (WHERE (sla_deadline IS NULL OR sla_deadline > NOW() + INTERVAL '3 hours') AND status NOT IN ('resolved','closed')) AS within_sla,
          COUNT(*) FILTER (WHERE sla_deadline >= NOW() AND sla_deadline <= NOW() + INTERVAL '3 hours' AND status NOT IN ('resolved','closed')) AS at_risk,
