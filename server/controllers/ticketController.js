...
exports.getTickets = async (req, res, next) => {
  try {
    ...
    if (slaStatus === 'breached') {
      ...
      conditions.push(`(t.sla_deadline >= NOW() AND t.sla_deadline <= NOW() + INTERVAL '3 hours' AND t.status NOT IN ('resolved','closed'))`);
    } else if (slaStatus === 'within') {
      conditions.push(`((t.sla_deadline IS NULL OR t.sla_deadline > NOW() + INTERVAL '3 hours') AND t.status NOT IN ('resolved','closed'))`);
    }
  ...
  try {
    ...
    const result = await query(`
      SELECT COUNT(*) FILTER (WHERE t.status = 'open') AS open_count,
             ...
             COUNT(*) FILTER (WHERE t.sla_deadline >= NOW() AND t.sla_deadline <= NOW() + INTERVAL '3 hours' AND t.status NOT IN ('resolved','closed')) AS sla_at_risk,
